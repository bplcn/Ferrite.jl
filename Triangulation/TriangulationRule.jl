...
function TriangulationRule{Tb}(::Type{RefQuadrilateral}, refinement_levels::Int) where {Tb <: Number}
    ...
    for j in 1:n
        ...
        for i in 1:n
            ...
            if i < n && j < n
                sub_nodes[j_offset + n + i] = T((coord_fun(i) + coord_fun(i + 1), coord_fun(j) + coord_fun(j + 1))) / 2
            end
        end
    end
    ...
    return TriangulationRule{Tb}(sub_nodes, triangles, edge)
end
...
function TriangulationRule{Tb}(::Type{RefTriangle}, refinement_levels::Int) where {Tb <: Number}
    ...
    edges = Vector{Int}(undef, n_segments * 3 + 1)
    fill!(edges, -1)
    cnt = 1
    for j in 0:n_segments
        v0 = v_base + dx_left * j
        edges[3 * n_segments + 1 - j] = cnt
        for i in 0:(n_segments - j)
            sub_nodes[cnt] = v0 + i * dx_horizontal
            j == 0 && (edges[i+1] = cnt)
            cnt += 1
        end
        edges[n_segments + 1 + j] = cnt - 1
    end
